...
dotenv.config();
const { TG_API_ID, TG_API_HASH, PHONE_NUMBER, SESSION_NAME, MY_USERNAME, LLM_CONTEXT_PATH, COMMAND_BASE_URL, } = process.env;
...
async function sendPromptAndGetResponse(command, question, context = '') {
    ...
    try {
        const response = await axios.post(endpoint, { question, context, ...params }, { headers: { 'Content-Type': 'application/json' } });
...
async function handleNewMessage(event) {
    ...
    if ((_a = msg === null || msg === void 0 ? void 0 : msg.peerId) === null || _a === void 0 ? void 0 : _a.channelId) {
        entityId = Number(msg.peerId.channelId.value);
        messageType = 'channel';
    }
    // Check for user messages (direct or group)
    else if ((_b = msg === null || msg === void 0 ? void 0 : msg.peerId) === null || _b === void 0 ? void 0 : _b.userId) {
    ...
    else if ((_c = msg === null || msg === void 0 ? void 0 : msg.peerId) === null || _c === void 0 ? void 0 : _c.chatId) {
    ...
    if (parsedCommand) {
        try {
            const llmcontext = await fs.readFile(LLM_CONTEXT_PATH, 'utf8'); // Read context here
            const response = await sendPromptAndGetResponse(parsedCommand.command, parsedCommand.content, llmcontext // Pass the context to the function
            );
    ...
    for (const filename in messagesMap) {
        ...
        if (messages.length > 0) {
            try {
                ...
                await fs.writeFile(`./logs/${filename}`, JSON.stringify(allMessages, (key, value) => typeof value === 'bigint' ? value.toString() : value, 2));
...
(async () => {
    ...
    if (await login()) {
        ...
        client.addEventHandler(handleNewMessage, new NewMessage({ incoming: true, forwards: false }));
